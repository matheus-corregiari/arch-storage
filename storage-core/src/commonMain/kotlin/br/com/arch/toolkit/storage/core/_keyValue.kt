...
abstract class KeyValue<DATA> {
    ...
    /** Scope used by writes that do not supply one explicitly. */
    protected var scope: CoroutineScope = defaultScope()
        private set

    /** Most recently cached value; it may be stale until the backend is observed. */
    abstract var lastValue: DATA
        protected set

    /** Observes values from the backend. Nullable entries emit null when absent. */
    ...
    /** Provides property delegation using platform-specific synchronous reads and ordinary writes. */
    ...
    /** Adapters for nullability, defaults and bidirectional mapping. */
    companion object {
        /** Requires a present value; reads fail when the source is null. */
        fun <T> KeyValue<T?>.required(): KeyValue<T> = RequiredKeyValue(
            keyValue = this,
            default = null
        )

        /** Requires a value, using [default] when the source is null without persisting it. */
        fun <T> KeyValue<T?>.required(default: () -> T): KeyValue<T> = RequiredKeyValue(
            keyValue = this,
            default = default
        )

        /** Supplies a nullable fallback without persisting it. */
        fun <T> KeyValue<T?>.default(default: T?): KeyValue<T?> = default { default }

        /** Computes a nullable fallback on reads without persisting it. */
        fun <T> KeyValue<T?>.default(default: () -> T?): KeyValue<T?> = DefaultKeyValue(
            keyValue = this,
            default = default
        )

        /** Maps reads with [mapTo] and writes with [mapBack], including nullable writes. */
